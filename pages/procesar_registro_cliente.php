...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (strlen($nombre) < 3 || !filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($password) < 6) {
        header("Location: registro.html?error=datos_invalidos");
        exit;
    }
    ...
    if ($edad < 18) {
        header("Location: registro.html?error=edad_minima");
        exit;
    }
    ...
    try {
        ...
        // Insertar usuario
        $stmt = $pdo->prepare("
            INSERT INTO usuarios (nombre, email, telefono, fecha_nacimiento, password)
        ...
        if ($usuario) {
            ...
            header("Location: registro.html?error=servidor");
            exit;
        }
